# userbot_manager.py (FINAL: Premium Emoji Fix + Safe Entity Parsing)
...
def send_cycle_start_notification(owner_id, userbot_name):
    try:
        Bot(token=config.BOT_TOKEN).send_message(owner_id, f"🤖 Userbot *{userbot_name}* Anda akan memulai siklus penyebaran pesan.", parse_mode=ParseMode.MARKDOWN)
    except Exception:
        pass

def log_to_channel(level, title, message):
    icons = {'SUCCESS': '✅', 'ERROR': '❌', 'WARNING': '⚠️', 'INFO': 'ℹ️', 'SUMMARY': '📊'}
    icon = icons.get(level, '⚙️')
    formatted_text = f"{icon} *{level}: {title}*\n\n{message}"
    try:
        Bot(token=config.BOT_TOKEN).send_message(chat_id=config.LOG_CHANNEL_ID, text=formatted_text, parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True)
    except Exception as e:
        log_system(f"❌ Gagal mengirim log ke channel: {e}")
    ...
    for _, owner_id, session_string in claimed_bots:
        try:
            async with Client(":memory:", api_id=config.API_ID, api_hash=config.API_HASH, session_string=session_string) as temp_app:
                ud = await temp_app.get_me()
                database.update_userbot_details(session_string, ud.id, ud.first_name)
                log_system(f"✅ Userbot '{ud.first_name}' diaktifkan.")
        except Exception as e:
            database.set_userbot_error(session_string, str(e))
            log_system(f"❌ Aktivasi GAGAL untuk owner {owner_id}.")
...
async def promo_handler(client, message):
    ...
    promo_config = database.get_jaseb_config(userbot_id)
    if not promo_config or not promo_config.get('promo_status') or not promo_config.get('promo_keywords') or not message.text:
        return
...
async def pm_auto_reply_handler(client, message):
    userbot_id = client.me.id
    cfg = database.get_jaseb_config(userbot_id)
    if cfg and cfg.get('pm_reply_status') and cfg.get('pm_reply_text'):
        try:
            await message.reply_text(cfg['pm_reply_text'], quote=True)
...
async def jaseb_worker(session_string, userbot_id, owner_id, userbot_name):
    app = None
    status = userbot_statuses.setdefault(userbot_id, {})
    ...
    try:
        ...
        app.add_handler(MessageHandler(promo_handler, filters.group & filters.text & ~filters.me))
        app.add_handler(MessageHandler(pm_auto_reply_handler, filters.private & ~filters.me))
        await app.start()
        me = await app.get_me()
        userbot_name = me.first_name
        status["name"] = userbot_name
        status["status"] = "🟢 Online"
        log_to_channel('SUCCESS', 'Userbot Online', f"Userbot `{userbot_name}` (`{userbot_id}`) berhasil terhubung.")
        database.add_jaseb_log(userbot_id, f"✅ Userbot '{userbot_name}' berhasil online.")

        while True:
            if not database.is_user_subscribed(owner_id):
                status["status"] = "⚠️ Langganan Habis"
                log_to_channel('WARNING', 'Langganan Berakhir', f"Userbot `{userbot_name}` dihentikan (owner `{owner_id}`).")
                database.add_jaseb_log(userbot_id, "⚠️ Langganan berakhir, userbot dihentikan.")
                break
            
            jaseb_config = database.get_jaseb_config(userbot_id)
            if not jaseb_config or not jaseb_config.get('running'):
                status["status"] = "⏹️ Berhenti (Idle)"
                await asyncio.sleep(15)
                continue
            
            send_cycle_start_notification(owner_id, userbot_name)
            delay_per_group = jaseb_config.get('delay', 20)
            status["status"] = "🔄 Memulai Siklus"
            database.add_jaseb_log(userbot_id, "🔄 Memulai siklus penyebaran pesan...")
            all_targets = []
            status["status"] = "🎯 Sinkronisasi"
            try:
                async for dialog in app.get_dialogs():
                    if dialog.chat.type in [ChatType.GROUP, ChatType.SUPERGROUP]:
                        all_targets.append(dialog.chat.id)
            except Exception as e:
                status["status"] = f"❌ Gagal Sync: {e}"
                await asyncio.sleep(60)
                continue
            
            banned_ids = set(database.get_banned_group_ids(userbot_id))
            targets = [tid for tid in all_targets if tid not in banned_ids]
            ...
            for i, target_id in enumerate(targets):
                ...
                if not current_config or not current_config.get('running'):
                    status["status"] = "⏹️ Dihentikan"
                    log_system(f"Siklus '{userbot_name}' dihentikan pengguna.")
                    break
                try:
                    msg_type = current_config['type']
                    text = current_config['text']
                    file_id = current_config['file_id']
                    entities_data = current_config.get('message_entities')

                    # === FIX: Parsing Premium Emoji & Entities ===
                    entities = []
                    if entities_data:
                        for ent in entities_data:
                            try:
                                ...
                                if isinstance(ent_type, str):
                                    ent_type = MessageEntityType[ent_type.split(".")[-1].upper()]
                                entity_obj = MessageEntity(
                                    type=ent_type,
                                ...
                                log_system(f"⚠️ Gagal parsing entity: {perr}")

                    if msg_type == 'text':
                        await app.send_message(target_id, text, entities=entities or None)
                    elif msg_type == 'photo':
                        await app.send_photo(target_id, file_id, caption=text, caption_entities=entities or None)
                    elif msg_type == 'video':
                        await app.send_video(target_id, file_id, caption=text, caption_entities=entities or None)
                    ...
                    error_count += 1
                    database.add_banned_group(userbot_id, target_id, type(e).__name__)
                    log_sending(f"❌ [red]{userbot_name}[/red] -> Gagal ({type(e).__name__}) di {target_id} -> di-skip")
                except errors.FloodWait as e:
                    status["status"] = f"⏳ Flood Wait ({e.value}s)"
            ...
            database.add_jaseb_log(userbot_id, f"📊 Siklus selesai. Berhasil: {sent_count}, Gagal: {error_count}.")
        ...
        if userbot_id in userbot_statuses:
            del userbot_statuses[userbot_id]
...
async def worker_manager():
    ...
    while True:
        try:
            await activate_pending_userbots()
            idle_bots = database.get_idle_active_userbots()
            ...
            log_system(f"💥 Error fatal di loop manager: {e}")
            await asyncio.sleep(60)

async def main_dashboard():
    database.reset_all_worker_statuses()
    log_to_channel('INFO', 'Sistem Dimulai', "Worker manager dan dasbor telah dimulai.")
